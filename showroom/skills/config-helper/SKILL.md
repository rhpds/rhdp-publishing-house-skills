...
See @showroom/skills/config-helper/references/showroom-patterns.md for the three lab patterns and their complete config examples.
See @showroom/skills/config-helper/references/config-files.md for the full reference on every config file.
...
Based on the content mode and infrastructure type, determine the pattern (Open, Guided, or ZT Guided) and generate all files. Use the exact templates from @showroom/skills/config-helper/references/showroom-patterns.md — do not improvise config structure.
...
1. Read existing config files to detect the current pattern. Use the detection rules from @showroom/skills/config-helper/references/showroom-patterns.md.
...
4. After modifications, suggest running `showroom:config-reviewer` to validate the result.
