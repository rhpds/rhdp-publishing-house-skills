...
Writes `validate.yml` — a playbook using `rhpds.ftl.validation_check` that verifies the student has completed each automatable step.
...
| Analysis triggered (async) | Any analysis task with Succeeded state (use `any()` not `max()`) |
| Extension installed | Directory exists in extensions path |
| Route deployed | Route exists and is reachable |
...
**IMPORTANT:** Always use `rhpds.ftl.validation_check` (fully qualified collection name). The bare name `validation_check` fails without collection routing.
...
**Playwright UI check (in-runner, no exec needed):**
```yaml
- name: Playwright — verify <page> loads
    ...
    argv:
      ...
      - |
        ...
        (async () => {
          ...
          if (!title.toLowerCase().includes('<expected>')) throw new Error('Unexpected title: ' + title);
          await p.screenshot({path: '/tmp/playwright-evidence-<module>.png', fullPage: false});
...
# _task_ok: "{{ r_playwright is defined and r_playwright is not failed and r_playwright is not skipped and r_playwright.rc | default(1) == 0 }}"
```
...
8. **FQCN always** — `rhpds.ftl.validation_check` not bare `validation_check`.
